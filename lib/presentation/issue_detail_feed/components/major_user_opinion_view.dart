...
class _MajorUserOpinionViewState extends State<MajorUserOpinionView>
  ...
  //       style: TextStyle(
  //         color: isSelected ? _biasColors[index] : AppColors.gray400,
  ...
  _buildTabViewPage({required Bias bias}) {
    final List<MajorComment> majorComments = widget.viewModel.getMajorComments(
      bias: bias,
    );
    return widget.viewModel.state.isMajorCommentsLoading
        ? Center(child: CircularProgressIndicator())
        : majorComments.isEmpty
        ? bias == Bias.center
            ? Row(
          ...
          children: List.generate(majorComments.length, (index) {
            ...
            return Padding(
              padding: EdgeInsets.only(
                bottom:
                    index == majorComments.length - 1 ? 0 : MyPaddings.large,
              ),
              child: MajorUserOpinionComponent(
                comment: comment,
  ...
  Widget build(BuildContext context) {
    return SectorBox(
      content: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SectorTitle(title: '성향별 사용자 대표 의견', icon: Icons.people_outline, onTap: widget.spreadCallback, isSpread: widget.isSpread),
          if (widget.isSpread)
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  AutoSizedTabBarView(
                    ...
                    children: [
                      _buildTabViewPage(bias: Bias.left),
                      _buildTabViewPage(bias: Bias.center),
                      _buildTabViewPage(bias: Bias.right),
                    ],
                  ),
                ],
              ),
